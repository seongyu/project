<template>
  <b-container>
    <b-row
      ...
      :key="title"
      v-show="target[title]==null?false:true"
    >
      ...
      <b-col sm="9">
        <b-form-input
          ...
          v-bind:readonly="vuetype=='update' ? title=='registerer' || title=='mac_address' || title=='registered_date'? true : readonly : readonly"
...
export default {
  ...
  async mounted() {
    ...
    var _url, result, _data, _parse, _raw;
    try {
      _url = [this.$apiUrl, "/device/", this.$route.params.id].join("");
      result = await this.$http.get(_url);
      _data = result.data.data[0];
      ...
      this.target.registered_date = moment(
        _util.timeparser(_data.registered_time)
      ).format("YYYY-MM-DD");
      if (_data.data && _data.data != "") {
        _parse = JSON.parse(_data.data);
        ...
        this.target.installed_date = _parse.installed_date
          ? _parse.installed_date
          : "";
        this.target.installed_location = _parse.installed_location
      ...
      if (this.type == "Beacon") {
        ...
        _raw = _raw.data.find(_=>{return _.mac==_data.device_id})
        if(_raw){
            this.target.rssi = _raw.rssi;
            this.target.version = _raw.majorVer + ':' + _raw.minorVer,
            this.target.uuid = _raw.UUID;
            this.target.battery_level = _raw.battLevel + '%';
        }
      }
    } catch (e) {
        console.log(e)
  ...
  methods: {
    async onSubmit() {
      ...
      item.installed_location
        ? (_data.installed_location = item.installed_location)
        : null;
